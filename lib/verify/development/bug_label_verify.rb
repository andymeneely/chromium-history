...
class BugLabelVerify < VerifyBase
  ...
  def verify_label_17941_count
    assert_equal 10, BugLabel.where(bug_id: 17941).count
  end
  
  def verify_label_17941_count_2
    assert_equal 10, Bug.find(17941).labels.count
  end
  ...
  def verify_os_all_bug_ids
    assert_equal [17941,27675,362405],Bug.joins(:labels).where("labels.label" => 'os-all').pluck(:bug_id)
  end
