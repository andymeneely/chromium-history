...
class SheriffRotationVerify < VerifyBase
  ...
  def verify_sheriff_rotation_start_dates
  ...
  def verify_sheriff_rotation_title
  ...
  def verify_sheriff_rotation_duration
  ...
  def verify_sheriff_rotations
    dev_id = Developer.where(email: 'danakj@chromium.org').pluck(:id)
    assert_equal 3, SheriffRotation.where(dev_id: dev_id).pluck(:dev_id).length
  end

  def verify_sheriff_hours
    assert_equal 96, Participant.find_by(issue: 23454014).sheriff_hours 
  end
end
