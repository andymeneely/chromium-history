...
class ForeignKeyVerify < VerifyBase
  ...
  def verify_code_reviews_and_cves_relationship
    ...
    get_results(__method__, error_count, "code_reviews", "cve")
  end
  ...
  def verify_patch_set_files_and_patch_sets_relationship
    ...
    get_results(__method__, error_count, "patch_set_files", "patch_set_id")
  end
  ...
  def verify_patch_sets_and_code_reviews_relationship
    ...
    get_results(__method__, error_count, "patch_sets", "code_review_id")
  end

  private
  def get_results(verify_name, error_count, table, foreign_column)
    if error_count == 0
      return_result(verify_name, true)
    else
      return_result(verify_name, false, "#{error_count.to_s} inconsistent #{foreign_column} in the #{table} table")
    end
  end
